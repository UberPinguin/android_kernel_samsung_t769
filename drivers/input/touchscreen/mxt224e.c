...
#if 1 // 20111212 add "if 1"
#define	T48_CALCFG                0x42
#else // 20111212 add "else"
...
//static struct t48_median_config_t noise_median; 



static struct t48_median_config_t noise_median = {0}; //110927 gumi noise
...
#if defined (CONFIG_KOR_MODEL_SHV_E110S) || defined (CONFIG_USA_MODEL_SGH_I727) ||defined (CONFIG_USA_MODEL_SGH_T989) || defined (CONFIG_USA_MODEL_SGH_T769)
...
#if 0 // 20111226 add "if 1"
static unsigned int resume_idle_flag = 1;
#endif
...
#if 0 // 20111226 add "if 1"
		if(resume_idle_flag == 1 && ta_status)
		{
			ret = get_object_info(copy_data, GEN_POWERCONFIG_T7, &size_one, &obj_address);
			value = 255;
			write_mem(copy_data, obj_address+0, 1, &value); 
			printk(KERN_ERR"[TSP] mxt224_ta_probe GEN_POWERCONFIG_T7 idleValue is %d\n",value);

			ret = get_object_info(copy_data, PROCG_NOISESUPPRESSION_T48, &size_one, &obj_address);
			value = 10;
			write_mem(copy_data, obj_address+27, 1, &value);		
			value = 70;//noise_median.t48_thr_for_mferr;
	...
	/* read_mem(diag_address, 82,data_buffer); */
		...
		/* write_mem(command_processor_address + DIAGNOSTIC_OFFSET, 1, &data_byte); */
			...
			if  ((tch_ch > 0) && (atch_ch == 0)) {  /* jwlee change. */
	                /* cal was bad - must recalibrateand check afterwards */
...
//
		...
		#if 0 // 20111226 add "if 1"
					if(resume_idle_flag == 2)
					{
						ret = get_object_info(copy_data, GEN_POWERCONFIG_T7, &size, &object_address);
						val = 48;
						write_mem(copy_data, object_address+0, 1, &val); 
						printk(KERN_ERR"[TSP] Check chip GEN_POWERCONFIG_T7 idleValue is %d\n",val);


						ret = get_object_info(copy_data, PROCG_NOISESUPPRESSION_T48, &size, &object_address);
						val = 20;
						write_mem(copy_data, object_address+27, 1, &val);											
						val = 60;
						write_mem(copy_data, object_address+35, 1, &val);
						val = 47;
						write_mem(copy_data, object_address+39, 1, &val);

						get_object_info(copy_data, SPT_CTECONFIG_T46, &size, &object_address);
						val = 38;
						write_mem(copy_data, object_address+3, 1, &val);
						resume_idle_flag = 0;
					}
#endif 			
	...
	if(!ta_status_check)
	{
		...
		switch(gErrCondition)
		{
			case ERR_RTN_CONDITION_T9:
			{
				ret = get_object_info(copy_data, TOUCH_MULTITOUCHSCREEN_T9, &size_one, &obj_address);
				value = 16;
				write_mem(copy_data, obj_address+6, 1, &value);  // gain 32=>16
				value = 40;
				write_mem(copy_data, obj_address+7, 1, &value);   // threshold 50 => 40
				value = 80;
				write_mem(copy_data, obj_address+13, 1, &value);  // move
				ret = get_object_info(copy_data, SPT_CTECONFIG_T46, &size_one, &obj_address);
				value = 32;
				write_mem(copy_data, obj_address+3, 1, &value);
				ret = get_object_info(copy_data, PROCG_NOISESUPPRESSION_T48, &size_one, &obj_address);
				value = 29;
				write_mem(copy_data, obj_address+3, 1, &value);  // base fre.
				value = 1;
				write_mem(copy_data, obj_address+8, 1, &value);  // median filter fre.
				value = 2;
...
static irqreturn_t mxt224_irq_thread(int irq, void *ptr)
{
    ...
    if((palm_chk_flag == 2)&& (data->family_id==0x80)) {
		palm_recovery();
	}
...
#if 1 // 20111212 add "if 1"
			if((cal_check_flag == 0)&&(data->family_id==0x80))
			{
				palm_recovery();
				cal_check_flag = 1u;
			}
...
*/
	...
	noise_median.median_on_flag = false;
	noise_median.mferr_count = 0;
	noise_median.t46_actvsyncsperx_for_mferr = 38;//63;
	noise_median.t48_mfinvlddiffthr_for_mferr = 12;//20;
	noise_median.t48_mferrorthr_for_mferr = 19;//22;
	noise_median.t48_thr_for_mferr = 45;
	noise_median.t48_movfilter_for_mferr = 0;//80
#endif	
