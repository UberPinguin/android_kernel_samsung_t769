...
static void mxt224_power_on(void)
{
	...
	rc = gpio_request(TSP_SDA, "tsp_irq");
	if(!rc)
		rc = gpio_direction_input(TSP_SDA);
	if (rc) {
		printk(KERN_NOTICE "%s: failed to set  TOUCHSCREEN_SDA to INPUT\n");
		}
	gpio_free(TSP_SDA);
	
	rc = gpio_request(TSP_SCL, "tsp_irq");
	if(!rc)
		rc = gpio_direction_input(TSP_SCL);
	if (rc) {
		printk(KERN_NOTICE "%s: failed to set  TOUCHSCREEN_SCL to INPUT\n");
		}
	gpio_free(TSP_SCL);
...
static void mxt224_power_off(void)
{
	...
	rc = gpio_request(TSP_SDA, "tsp_irq");
	if(!rc)
		rc = gpio_direction_output(TSP_SDA, 1);
	if (rc) {
		printk(KERN_NOTICE "%s: failed to set  TOUCHSCREEN_SDA to OUTPUT(1)\n");
	}
	gpio_free(TSP_SDA);

	rc = gpio_request(TSP_SCL, "tsp_irq");
	if(!rc)
		rc = gpio_direction_output(TSP_SCL, 1);
	if (rc) {
		printk(KERN_NOTICE "%s: failed to set  TOUCHSCREEN_SCL to OUTPUT(1)\n");
	}
	gpio_free(TSP_SCL);
...
static struct 
	qt602240_platform_data qt602240_pdata = {
	.x_line                 = 19,
	.y_line                 = 11,
	.x_size                 = 480,
	.y_size                 = 800,
...
#if defined(CONFIG_KOR_MODEL_SHV_E160S) || defined(CONFIG_KOR_MODEL_SHV_E160K) || defined(CONFIG_KOR_MODEL_SHV_E160L)
	for (pin= 0; pin < ARRAY_SIZE(p6_wacom_gpio_sleep); pin++) {
		rc = gpio_tlmm_config(p6_wacom_gpio_sleep[pin], GPIO_CFG_ENABLE);
		if (rc) {
			printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n",	__func__, p6_wacom_gpio_sleep[pin], rc);
		}
	}
#endif
...
#if defined(CONFIG_KOR_MODEL_SHV_E160S) || defined(CONFIG_KOR_MODEL_SHV_E160K) || defined(CONFIG_KOR_MODEL_SHV_E160L)
	for (pin= 0; pin < ARRAY_SIZE(p6_wacom_gpio_wake); pin++) {
		rc = gpio_tlmm_config(p6_wacom_gpio_wake[pin], GPIO_CFG_ENABLE);
		if (rc) {
			printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n",	__func__, p6_wacom_gpio_wake[pin], rc);
		}
	}
#endif
...
static struct platform_device *surf_devices2[] __initdata = {
  ...
  || defined(CONFIG_USA_MODEL_SGH_I717) || defined(CONFIG_USA_MODEL_SGH_I757) || defined(CONFIG_CAN_MODEL_SGH_I757M) || defined (CONFIG_USA_MODEL_SGH_T769) || defined(CONFIG_USA_MODEL_SGH_I577) || defined(CONFIG_USA_MODEL_SGH_T769)
	&msm_gsbi1_qup_i2c_device,	
#endif	
...
#endif /*CONFIG_TOUCHSCREEN_MXT224E */
#if defined(CONFIG_TOUCHSCREEN_ATMEL_MXT540E)
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,
		MSM_GSBI3_QUP_I2C_BUS_ID,
...
#elif defined (CONFIG_KOR_MODEL_SHV_E120S) || defined (CONFIG_KOR_MODEL_SHV_E120K) || defined(CONFIG_KOR_MODEL_SHV_E160S) || defined(CONFIG_KOR_MODEL_SHV_E160K) || defined(CONFIG_USA_MODEL_SGH_I757) || defined(CONFIG_CAN_MODEL_SGH_I757M) || defined(CONFIG_USA_MODEL_SGH_I577)
	printk( MIPI_STR "%s: Enable=%d, SKT/KT seq\n", __func__, enable);
	isUse_LDO3 = true;
...
int tkey_vdd_enable(int onoff)
{
	...
	if(onoff)  {
		...
		if(regulator_is_enabled(lvs2)){
			...
			if (ret) 
				printk("[TKEY] %s: error enabling regulator (lvs2)\n", __func__);
			else 
				printk("[TKEY] %s: lvs2 is OFF  !!!\n", __func__);
        	} else{
            		printk("[TKEY] %s: TKEY is already OFF (lvs2) !!!\n", __func__);
        	}
 	}
